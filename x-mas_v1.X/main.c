...
void Myusartinit()
{
	...
	//setting TX/RX
	PIR1bits.TXIF =
	    1; //1 = The EUSART transmit buffer, TXREG, is empty (cleared when TXREG is written), 0 = The EUSART transmit buffer is full
	PIR1bits.RCIF =
	    0; //1 = The EUSART receive buffer, RCREG, is full (cleared when RCREG is read), 0 = The EUSART receive buffer is empty
	TXSTAbits.TXEN = 1;          //Enable Tx
	RCSTAbits.CREN = 1;            //Enable Rx
	//setting TX/RX interrupt
	PIE1bits.TXIE = 0;              //Tx interrupt
	IPR1bits.TXIP =
	    1;              //Setting Tx as high/low priority interrupt 1 high, 0 low
	PIE1bits.RCIE = 1;              //Rx interrupt
	IPR1bits.RCIP =
	    1;              //Setting Rc as high/low priority interrupt 1 high, 0 low
